...
- `ScoreCardApp.makeModelContainer()` builds a persistent container with `cloudKitDatabase: .automatic` (iCloud sync), falling back to a local-only store if CloudKit is unavailable. The container is held in a stored property — a `ModelContext` does **not** retain its container, so any code (including tests) that uses a context must keep the container alive. Two launch arguments adjust startup: `-uitesting` switches to a clean in-memory store (used by `ScoreCardUITests`); `-seedSampleData` runs `SampleDataSeeder.seedIfNeeded(...)`, a one-time (UserDefaults-guarded, at most once per install) seed of realistic players/teams/games into the *live* store.
