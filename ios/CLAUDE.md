...
- **Action log** (`Services/ActionLog.swift` + `ActionLogRecorder.swift`, specified in `docs/action-log.md`). `ActionLogRecorder` observes `ModelContext.willSave`/`didSave` and records every store mutation, so no mutation site has to remember to log. Two orderings in there are load-bearing and easy to break: the notifications are registered with **`queue: nil`** so the block runs synchronously on the posting thread (an `OperationQueue` defers it until after the save, when the deleted objects are already gone), and **deletions are written at `willSave`** — while their names and relationships can still be read — while **creations and changes are held until `didSave`**, because a freshly inserted object's `persistentModelID` changes on its first save and an id captured earlier would never match the lines that follow it. The log is a rolling pair of files in Application Support, deliberately not a `@Model` (`eraseAll` would destroy it) and deliberately not in the iCloud Documents folder (it must not sync); it is not part of `BackupSnapshot`. The destructive views additionally call `ActionLogRecorder.note(...)` before acting, because the store-level hook cannot tell a local edit from a CloudKit merge.
