...
- **Domain model semantics** — the entity set (Player, Team, Game, GameParticipant, ScoreEntry, Seat, GameEdit) and rules like "a win is the top score in a closed game, ties count for all leaders", dealer rotation order (counter-clockwise from `position` 0; the deal direction and the after-a-draw rule are app-wide preferences — see `docs/dealing-rules.md`), the below-zero scoring policy (an app-wide preference that clamps subtraction at zero by default — see `docs/scoring-rules.md`), editing a finished game (scores only, behind a mandatory reason, recorded as adjustment entries plus a `GameEdit` — see `docs/game-editing.md`), and the "Next Hand" / target-lock scoreboard behaviors are product decisions, not platform details. The iOS app is the reference implementation; when porting, match its behavior unless a doc in `docs/` says otherwise.
