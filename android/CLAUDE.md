...
- **Compose UI tests** live alongside them. `HelpUiTest` pins the Settings entry point into the help page and that a topic opens — the entry point, deliberately not the prose, which `HelpContentTest` covers on the JVM. `GameEditingUiTest` pins the closed-game editor's mandatory-reason gate and the "Edited" badge/history — the rules a refactor could drop without failing anything else. Two conventions worth keeping: a UI test asserts the invariant by *attempting the bypass* (clicking a disabled `Continue` and checking the scores step still does not exist), not merely by asserting a button's enabled state, because a greyed-out control that still works when tapped would pass the weaker check; and score changes are driven through the existing `Raise <name>'s total` / `Lower <name>'s total` content descriptions rather than text entry. `AppContainer` takes an optional `database` so these run against `Room.inMemoryDatabaseBuilder` and never touch the games on the device — production never passes it.
